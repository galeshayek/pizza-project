...
const EditRecipe = () => {
    ...
    if (id) {
      recipeSerivce.getRecipeById(id).then((res) => {
        reset({
          ...
          "info.portions": res.data.info.portions,
          "info.amount": res.data.info.amount,
        });
        setAmount(res.data.info.amount);
        setPortions(res.data.info.portions);
      });
    }
  }, [id, reset]);
  const onSubmit: SubmitHandler<ICreateRecipe> = (data) => {
    ...
    if (jwt && id) {
      recipeSerivce.updateRecipe(jwt, id, filteredData).then((res) => {
        console.log(res);
        if (res.status === 202) {
          reset();
          Swal.fire({
            title: "Recipe Updated succesfully",
            icon: "success",
            confirmButtonText: "Go to Recipe",
          })
            .then((res) => {
              if (res.isConfirmed) {
                navigate(`/recipe/single/${id}`);
              }
            })
            .catch((e) => {
  ...
  return (
    <section className="mt-10 px-24">
      <h2 className="mb-5 text-primary">Update Recipe</h2>
      <form
